...
extern "efiapi" fn get_memory_attributes(
    ...
    match dxe_services::core_get_memory_space_descriptor(base_address) {
        Ok(descriptor) => {
            ...
            // Ensure that the requested range is a subset of the descriptor range
            if !(req_range.start >= desc_range.start && req_range.end <= desc_range.end) {
                log::error!(
                    "{} Requested range [{:#x}, {:#x}) is not a subset of descriptor range [{:#x}, {:#x})",
                    function!(),
                    req_range.start,
                    req_range.end,
                    desc_range.start,
